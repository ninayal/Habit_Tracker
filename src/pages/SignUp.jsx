...
export default function SignUp() {
  ...
  const handleSubmit = (e) => {
    ...
    // Touch tất cả fields
    const allFields = { fullName: true, email: true, password: true, confirmPassword: true, terms: true };
    setTouched(allFields);

    if (Object.keys(errors).length > 0) return;

    const result = authService.register(form.fullName, form.email, form.password);
    if (result.success) {
      navigate("/dashboard");
    } else {
  ...
  /* ── Render ── */
  return (
    <AuthCard title="Create Account" subtitle="Start building better habits today">
      {/* Server error banner */}
      {serverError && (
